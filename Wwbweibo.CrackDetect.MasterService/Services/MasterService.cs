...
namespace Wwbweibo.CrackDetect.MasterService.Services
{
    public class MasterService
    {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<Dictionary<ServiceType, List<Tuple<string, Wwbweibo.CrackDetect.Models.ServiceStatusEnum>>>> ListAllRegisteredService()
        {
            ...
            foreach (ServiceType serviceType in Enum.GetValues(typeof(ServiceType)))
            {
                ...
                var services = await zkClient.ListChildren(servicePath);
                if (services == null)
                    continue;
